...
export default function Home() {
  ...
  const featuredProjectClient = featuredProject ? getClientBySlug(featuredProject.client) : null;
  const featuredProjectImage = featuredProject ? getProjectCardImage(featuredProject) : undefined;
    ...
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-20 px-gutter pt-40 pb-20">
      ...
      {(featured || featuredProject) && (
        <section
          ...
          className="grid grid-cols-1 gap-gutter sm:grid-cols-2 sm:items-start"
        >
          {featured && (
            <Link
          ...
          {featuredProject && featuredProjectClient && (
            <ProjectCard
              title={featuredProject.title}
              summary={featuredProject.summary}
              clientName={featuredProjectClient.name}
              href={`/work/${featuredProjectClient.slug}`}
              imageSrc={
                featuredProjectImage
                  ? `/content-images/projects/${featuredProject.slug}/images/${featuredProjectImage}`
                  : undefined
              }
            />
          )}
